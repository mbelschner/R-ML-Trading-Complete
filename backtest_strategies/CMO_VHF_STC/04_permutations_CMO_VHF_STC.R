...
# 15-Minuten Intraday Trading mit ATR Risk Management
...
cat("########################################################################\n")
cat("#                                                                      #\n")
cat("#      MCPT für CMO-VHF-STC Strategie - ERWEITERTE VERSION            #\n")
cat("#      Mit ATR Risk Management & Momentum-Tests                       #\n")
cat("#                                                                      #\n")
cat("########################################################################\n\n")
...
cat("✓ CMO, VHF und STC berechnet\n")
cat("✓ ATR und ADX berechnet\n")
...
if (!is.null(train_trades_raw)) {
  ...
  p_equity <- ggplot(train_trades, aes(x = exit_time, y = capital_after)) +
    ...
    labs(
      title = "Equity Curve - Training (CMO-VHF-STC Strategy)",
      x = NULL,
      y = "Account Balance"
    ) +
    theme_minimal() +
...
p1 <- ggplot(df_plot_train, aes(x = metric)) +
  ...
  labs(
    title = "MCPT: TRAINING-DATEN (CMO-VHF-STC)",
...
if (nrow(test_data) > 0) {
  ...
  p2 <- ggplot(df_plot_test, aes(x = metric)) +
    ...
    annotate("text", 
             ...
             hjust = -0.1, vjust = 2, color = "red", size = 4) +
    labs(
      title = "MCPT: TEST-DATEN (CMO-VHF-STC)",
      subtitle = sprintf("p-Wert = %.4f | Metrik: %s | N = %d", 
...
p5 <- ggplot(train_perf$data, aes(x = date, y = equity_curve)) +
  ...
  labs(
    title = "In-Sample Equity Curve (CMO-VHF-STC)",
...
if (nrow(test_data) > 0) {
  ...
  p6 <- ggplot(test_perf$data, aes(x = date, y = equity_curve)) +
    ...
    geom_hline(yintercept = 1, linetype = "dashed", color = "red") +
    labs(
      title = "Out-of-Sample Equity Curve (CMO-VHF-STC)",
      subtitle = sprintf("Sharpe: %.2f | Return: %.2f%% | Trades: %d | p = %.4f",
                         test_perf$sharpe_ratio,
...
cat(sprintf("  STC Entry: Long>%d, Short<%d\n", STC_ENTRY_LONG, STC_ENTRY_SHORT))
cat(sprintf("  ATR: Period=%d, SL=%.1fx, TP=%.1fx\n", 
            ATR_PERIOD, STOP_LOSS_ATR_MULT, TAKE_PROFIT_ATR_MULT))
cat(sprintf("  Price Confirmation: %s\n", USE_PRICE_CONFIRMATION))
cat(sprintf("  ADX Filter: %s", USE_ADX_FILTER))
if(USE_ADX_FILTER) cat(sprintf(" (Threshold=%d)", ADX_THRESHOLD))
cat(sprintf("\n  Close Time: %d:00 Uhr\n\n", CLOSE_TIME_HOUR))
...
if (nrow(test_data) > 0 && !is.null(test_trades_raw)) {
  
  cat("\n")
  cat("========================================\n")
  cat("DETAILLIERTE TEST-PERFORMANCE\n")
  cat("========================================\n\n")
  
  start_row_test <- data.frame(
    trade_num = 0,
  ...
  test_trades <- rbind(start_row_test, test_trades)
  ...
  win_rate_calc <- winning_trades / (winning_trades + losing_trades) * 100
  avg_win <- mean(test_trades$net_pnl[test_trades$net_pnl > 0])
  avg_loss <- mean(test_trades$net_pnl[test_trades$net_pnl < 0])
  profit_factor_calc <- abs(sum(test_trades$net_pnl[test_trades$net_pnl > 0]) / 
                              sum(test_trades$net_pnl[test_trades$net_pnl < 0]))
  max_drawdown_calc <- min(test_trades$drawdown)
  ...
  # Equity Curve Test
  trades_profit_test <- test_trades %>% filter(capital_after >= 1000)
  trades_loss_test <- test_trades %>% filter(capital_after < 1000)
  
  p_equity_test <- ggplot(test_trades, aes(x = exit_time, y = capital_after)) +
    geom_ribbon(data = trades_profit_test, aes(ymin = 1000, ymax = capital_after), 
                fill = "green", alpha = 0.3) +
    geom_ribbon(data = trades_loss_test, aes(ymin = capital_after, ymax = 1000), 
                fill = "red", alpha = 0.3) +
    geom_line(color = "#2E86AB", linewidth = 1.2) +
    geom_hline(yintercept = 1000, linetype = "dashed", color = "gray50", alpha = 0.7) +
    scale_y_continuous(labels = dollar_format(prefix = "$")) +
    scale_x_datetime(date_labels = "%Y-%m", date_breaks = "1 month") +
    labs(
      title = "Equity Curve - Test (CMO-VHF-STC Strategy)",
      x = NULL,
      y = "Account Balance"
    ) +
    theme_minimal() +
